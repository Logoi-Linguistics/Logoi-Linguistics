# Logoi (<a target="_blank" href="https://en.wiktionary.org/wiki/%CE%BB%CF%8C%CE%B3%CE%BF%CE%B9">λόγοι</a>)

Logoi ≡ (<a target="_blank" href="https://en.wikipedia.org/wiki/Prolog">Prolog</a> ∧ <a target="_blank" href="https://en.wikipedia.org/wiki/Lisp_(programming_language)">Lisp</a>)
...
> [!NOTE]
> As of January 2025: Logoi's construction has been 99.9% mouseless. There is neither sound nor sane reason for this design decision, but inertia stubbornly remains, as is commonly known, a pervasive property of all matter in the cosmos (micro- & macro-), massive & otherwise.
...
MetaFunction
    ImpureFunctionAlias/Domain
    FunctionConstructor/Arity
        ParameterVariable
        ImpureFunctionAlias/Domain
            VariadicFunctionAlias/Domain
                TextualConstant
                ParameterVariable
                TextualConstant
...
From the very beginning, return type annotations were categorically eliminated from function signatures by simply asking Logoi authors to restrict the return type of their function to exist within a subset of that function's input types. Type conversion functions are thus employed to explicitly handle type mutations on a separate line so as to reduce Logoi's average line-length.
...
- Official Movie: [The Wild Robot](https://youtu.be/67vbA5ZJdKQ)
- Unofficial Movie: [Tropic Thunder](https://youtu.be/9Pl4JNnqNaE)
